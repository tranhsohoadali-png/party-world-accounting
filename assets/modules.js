...
M.productForm = function (p, opts) {
  ...
  const f = {
    ...
    barcode: C.input({ value: p.barcode || '', placeholder: 'mã vạch FAHASA (EAN)' }),
    barcodePN: C.input({ value: p.barcodePN || '', placeholder: 'mã vạch Phương Nam (EAN)' }),
  ...
  const body = U.el('div', null, [
    U.el('div', { class: 'form-grid' }, [
      ...
      C.field('Mã vạch FAHASA', f.barcode),
      C.field('Mã vạch Phương Nam', f.barcodePN),
  ...
  function saveProduct() {
    ...
    const obj = {
      ...
      barcode: f.barcode.value.trim(), barcodePN: f.barcodePN.value.trim(),
      kind: f.kind.value,
